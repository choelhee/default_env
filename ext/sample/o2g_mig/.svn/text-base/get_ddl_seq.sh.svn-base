...
select dbms_metadata.get_ddl( typ, nm, Case When typ NOT IN ( 'TABLESPACE' ) Then usr Else NULL End ) as DDL from ( select OBJECT_TYPE typ, OBJECT_NAME nm, OWNER usr from ALL_OBJECTS WHERE OWNER = UPPER('${OWN}') AND OBJECT_TYPE = 'SEQUENCE' ) a;
...
cat _${OFNM} |sed -e "s/^ [ ]*//g" |grep ^CREATE |sed -e "s/9999999999999999999999999999/9223372036854775807/g" |sed -e "s/ NOORDER / /g" > ${OFNM}
