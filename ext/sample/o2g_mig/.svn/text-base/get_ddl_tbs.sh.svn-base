...
select dbms_metadata.get_ddl( typ, nm, Case When typ NOT IN ( 'TABLESPACE' ) Then usr Else NULL End ) as DDL from ( select 'TABLESPACE' typ, TABLESPACE_NAME nm, NULL usr from DBA_TABLESPACES where TABLESPACE_NAME like 'TS_%' ) a;
...
cp ${OFNM} _${OFNM}
# cat _${OFNM} |grep -v "^SQL>" | sed -e "s/ LOGGING .*;/;/g" > ${OFNM}
# cat _${OFNM} |grep -v "^SQL>" | sed -e "s/ LOGGING .*;/;/g" |sed -e "s/ AUTOEXTEND ON.*;/;/g" > ${OFNM}
# cat _${OFNM} |grep -v "^SQL>" | sed -e "s/ LOGGING [^,]*MANAGEMENT AUTO//g" > ${OFNM}
# cat _${OFNM} |grep -v "^SQL>" | sed -e "s/ LOGGING [^,]*MANAGEMENT AUTO//g" |sed -e "s/ AUTOEXTEND ON [^,]*MAXSIZE [0-9][0-9]*[KMG]*//g" > ${OFNM}
...
# CREATE TABLESPACE "TS_MM_I" DATAFILE 'TS_MM_I_001.dbf' SIZE 10485760,'TS_MM_I_002.dbf' SIZE 10485760; /* ALTER DATABASE DATAFILE 'TS_MM_I_002.dbf' RESIZE 576716800; */
sed -i -e "s|ALTER DATABASE DATAFILE.*;|/* & */|g" ${OFNM}
