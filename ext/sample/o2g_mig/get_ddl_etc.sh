...
--show all
--select dbms_metadata.get_ddl( UPPER('${TYPE}'), '${NAME}', UPPER('${USER}') ) || ';' from dual;
--select dbms_metadata.get_ddl( typ, nm, Case When typ NOT IN ( 'TABLESPACE' ) Then usr Else NULL End ) as DDL from ( select UPPER('${TYPE}') typ, '${NAME}' nm, UPPER('${USER}') usr from dual ) a;
...
chk=`grep "^ORA-" ${OFNM} |wc -l`
test "x$chk" != "x0" && mv ${OFNM} ${OFNM}.bad
...
# run TEST_S1 SEQUENCE
# exit 0

run P1
run T1 TABLE

run TS_MM_I TABLESPACE
